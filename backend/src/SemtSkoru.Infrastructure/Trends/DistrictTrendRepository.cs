...
{
    public Task<DistrictTrendSummary?> GetAsync(string neighborhoodId, CancellationToken ct) =>
        db.DistrictTrendSummaries.AsNoTracking().FirstOrDefaultAsync(s => s.NeighborhoodId == neighborhoodId, ct);
}
