...
///     backfilled - the day this job first runs is the day this feature's history starts.
///
...
///     of the change. A district with no eligible baseline yet is skipped entirely - no Gemini
...
/// Core's inbound rate limiter has no idea this job exists) and skips districts whose comparison
/// basis hasn't changed since the last run (DistrictTrendSignature), so a run's real Gemini spend
/// is usually far below the worst-case figure the cadence in Program.cs is sized against.
/// </summary>
    ...
    TimeProvider timeProvider)
{
    ...
    // of the same budget, and a 30s backoff avoids hammering Gemini with the next 38 districts
    // immediately after it's already said to slow down.
        ...
        CancellationToken ct)
    {
        ...
        var existingTrends = await db.DistrictTrendSummaries.ToDictionaryAsync(t => t.NeighborhoodId, ct);
        ...
        foreach (var (id, name) in names.OrderBy(kv => kv.Value, StringComparer.CurrentCultureIgnoreCase))
        {
            ...
            var existing = existingTrends.GetValueOrDefault(id);
            ...
            if (baseline is null)
            {
                ...
                await RemoveStaleTrendIfAnyAsync(existing, ct);
                continue;
            }
            ...
            if (deltas.Count == 0)
            {
                ...
                // PREVIOUS run's trend text is still sitting there (comparing against an even
                // older baseline whose delta WAS meaningful at the time), it no longer reflects
                // this district's current standing and must not keep being shown - see
                // DistrictTrendBadge's "renders nothing" contract on the frontend.
                await RemoveStaleTrendIfAnyAsync(existing, ct);
                continue;
            }
            ...
            if (existing is not null && existing.ComparisonSignature == signature)
            {
                logger.LogInformation(
                    "Skipping district trend for {NeighborhoodId}: comparison unchanged since {GeneratedAt}",
                    id, existing.GeneratedAt);
                continue; // same meaningful deltas as last time - don't spend a Gemini call.
            }
            ...
            var outcome = await trendService.GenerateTrendAsync(name, deltas, ct);
            ...
            {
                logger.LogWarning("Gemini rate-limited district trend generation at {NeighborhoodId}; backing off", id);
            ...
            {
                logger.LogInformation("No usable district trend generated for {NeighborhoodId}: {Outcome}", id, outcome.Kind);
                continue;
            }
            ...
            // minutes, so a mid-run failure should keep whatever districts already succeeded
            // rather than lose the whole run's progress - mirrors DistrictSummaryGenerationJob.
            await db.SaveChangesAsync(ct);
        }
    }

    private async Task RemoveStaleTrendIfAnyAsync(DistrictTrendSummary? existing, CancellationToken ct)
    {
        if (existing is null)
        {
            return;
